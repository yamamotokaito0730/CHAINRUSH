...
public class CameraIntroMover : MonoBehaviour
{
    ...
    private IEnumerator CameraSequence()
    {
        ...
        //SetCameraInstantly(points[0]);
        yield return new WaitForSeconds(c1); // 少し止める演出
        SetCameraInstantly(points[1]);
        yield return new WaitForSeconds(c2); // 少し止める演出
        SetCameraInstantly(points[2]);
        yield return new WaitForSeconds(c3); // 少し止める演出
        yield return MoveCameraSmoothly2(points[2], CameraPos);
